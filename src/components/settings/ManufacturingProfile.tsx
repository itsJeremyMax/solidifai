 ...
 * the print/cut process, and advisory fabrication values. The `scope` prop fixes
 * the layer being edited: the global layer lives in App settings, the workspace
 * layer in Workspace settings. Workspace fields fall back to the global profile,
 * and every field shows whether it is set here or inherited, with a one-click reset
 * back to the inherited value. The default material is echoed read-only (it is
 * owned by the Materials library).
 *
...
export default function ManufacturingProfile({ scope }: { scope: Scope }) {
  // The global layer never needs a workspace; the workspace layer confirms one is
  // focused (this page is normally reached from within a workspace).
  const [hasWorkspace, setHasWorkspace] = useState(scope === "global");
  const [ready, setReady] = useState(scope === "global");
  ...
  if (scope === "workspace" && !hasWorkspace) {
    return (
      <div className="mx-auto max-w-160 py-6.5 text-body text-ink-3">
        Open a workspace to edit its manufacturing profile.
      </div>
    );
  }
  return <Panel scope={scope} />;
}
      ...
      {loading ? (
        ...
        <>
          <div className="mt-4 flex items-center gap-2 rounded-panel border border-line bg-surface px-4 py-2.5 text-body">
            ...
            <span className="font-medium text-ink">{view.material.label || "None set"}</span>
