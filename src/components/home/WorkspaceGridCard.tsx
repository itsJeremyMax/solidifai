...
export function WorkspaceGridCard({
  ...
  return (
    ...
    >
      {/* The whole card is the open affordance: one stretched button beneath the
          content. The menu, tag chips, and rename actions float above it as
          pointer-events islands, so they stay clickable without nesting inside it. */}
      <button
        type="button"
        onClick={onOpen}
        disabled={disabled}
      ...
      {/* Render tile — full-width, ~150px tall. Clicks pass through to open; the
          menu re-enables pointer events. */}
      <div className="pointer-events-none relative z-10 h-[150px]">
        <WorkspaceThumb name={ws.name} src={thumbSrc} className="h-full w-full" />

        {/* Archived badge — top-left over render */}
        {archived && (
          <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full border border-line-2 bg-white/90 px-2 py-0.5 text-micro font-bold uppercase tracking-[0.04em] text-ink-2 backdrop-blur-sm">
            Archived
          </span>
        )}
        ...
        {!archived && ws.proposedName && (
          <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full border border-amber-200/80 bg-amber-50/90 px-2 py-0.5 text-micro font-bold uppercase tracking-[0.04em] text-amber-700 backdrop-blur-sm">
            Rename suggested
          </span>
        )}
      ...
      {/* Card body — text passes clicks through to the open button; only the tag
          chips re-enable pointer events so they stay clickable. */}
      <div
        className={`pointer-events-none relative z-10 px-3.5 pt-2.75 ${
          !archived && ws.proposedName ? "pb-2" : "pb-3.5"
        }`}
      >
        {/* Name + spinner */}
      ...
      {/* Sol name suggestion — its buttons float above the open overlay. */}
      {!archived && ws.proposedName && (
        <div className="pointer-events-none relative z-10 flex items-center gap-1.5 border-t border-line bg-surface-2/60 px-3.5 py-2 text-caption text-ink-2">
          <span>Sol suggests</span>
          <span className="flex-1 truncate font-semibold text-ink">{ws.proposedName}</span>
          <button
            type="button"
            onClick={onAcceptName}
            className="pointer-events-auto rounded px-1.5 py-0.5 text-micro font-semibold text-accent transition-colors duration-100 hover:bg-accent-tint focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent-line"
          >
            Accept
          </button>
          <button
            ...
            className="pointer-events-auto rounded px-1.5 py-0.5 text-micro font-semibold text-ink-3 transition-colors duration-100 hover:bg-surface-2 hover:text-ink-2 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-line-3"
          >
            Dismiss
          </button>
        </div>
