...
class Session:
    ...
    def capture_views(
        ...
        if layout not in ("separate", "grid"):
            return {
                "ok": False,
                "error": f"unknown layout {layout!r}; expected 'separate' or 'grid'",
            }
        ...
        if (
            ...
            return {
                "ok": False,
                "error": f"resolution must be an integer in 256..2048, got {resolution!r}",
            }
        if section is not None:
            if explode and explode > 0:
                return {"ok": False, "error": "section cannot be combined with explode"}
            if (
                ...
                return {
                    "ok": False,
                    "error": 'section must be {"axis": "x"|"y"|"z", "offset_mm": <number>}',
                }
        ...
        try:
            ...
            # radially so a multi-part fit can be reviewed pulled apart (the
            # agent's equivalent of the viewport's Explode slider). exploded()
            # returns translated COPIES for off-center parts but the ORIGINAL
            # object for a concentric / unmovable one, so we must NOT wrap the
            # result in a Compound — that would reparent a shared original out of
            # self._model and corrupt it. Instead we merge per-part meshes.
            # Nothing here mutates self._model / self._objects, so the saved +
            # exported model stays assembled.
            spread = None
            if explode and explode > 0 and len(objects) > 1:
                spread = solidifai.exploded([o.shape for o in objects], explode)
            # Colored path: one display-sRGB base color per object (same resolver
            # the viewport uses). Falls back to clay when color is off or no
            # object snapshot exists.
            want_groups = bool(color and objects)
            groups: list | None = [] if want_groups else None
            if section is not None:
                # Plane-cut COPIES (the boolean never touches the live shapes),
                # under a hard timeout; compose per-object cut meshes exactly
                # like the explode path (concatenate, never re-Compound).
                ...
                try:
                    ...
                    return {
                        "ok": False,
                        "error": f"section cut timed out after {SECTION_TIMEOUT_S:.0f}s "
                ...
                if not vertices:
                    return {"ok": False, "error": "section removed the entire model"}
                if cap_faces:
                ...
                # Compound, no reparenting); build the colored groups from the
                # same per-part tessellation in one pass.
                vertices = []
                tris = []
        ...
        # Highlight validation: done before rendering so an unknown name is a
        # clean error with no partial render output.
        highlight_mesh = None
        if highlight:
            by_name = {f.name: f for f in self._features}
            ...
            if missing:
                valid = ", ".join(sorted(by_name)) or "(none)"
                return {"ok": False, "error": f"unknown feature(s) {missing}; valid: {valid}"}
            hfaces = [fc for h in highlight for fc in (by_name[h].faces or [])]
            hverts, htris = feature_geom.tessellate(hfaces, TESS_TOLERANCE)
            highlight_mesh = (hverts, htris) if hverts else None
        focus_bounds: tuple | None = None
        ...
        if focus is not None:
            if isinstance(focus, str):
                ...
                if rec is None:
                    ...
                    return {
                        "ok": False,
                        "error": f"unknown focus feature {focus!r}; valid: {valid}",
                    }
                summary = feature_geom.summarize(rec.faces or [])
                if not summary["bbox"]:
                    return {
                        "ok": False,
                        "error": f"focus feature {focus!r} has no geometry to frame",
                    }
                ...
                if len(vals) != 6 or any(vals[k] > vals[k + 3] for k in range(3)):
                    return {
                        "ok": False,
                        "error": "focus must be a feature name or "
                        "[xmin, ymin, zmin, xmax, ymax, zmax] with min <= max",
                    }
                focus_bounds = tuple(vals)
                focus_label = "region"
        try:
